
// To check if a library is compiled with CocoaPods you
// can use the `COCOAPODS` macro definition which is
// defined in the xcconfigs so it is available in
// headers also when they are imported in the client
// project.


// CocoaLumberjack
#define COCOAPODS_POD_AVAILABLE_CocoaLumberjack
#define COCOAPODS_VERSION_MAJOR_CocoaLumberjack 2
#define COCOAPODS_VERSION_MINOR_CocoaLumberjack 0
#define COCOAPODS_VERSION_PATCH_CocoaLumberjack 0

// CocoaLumberjack/CLI
#define COCOAPODS_POD_AVAILABLE_CocoaLumberjack_CLI
#define COCOAPODS_VERSION_MAJOR_CocoaLumberjack_CLI 2
#define COCOAPODS_VERSION_MINOR_CocoaLumberjack_CLI 0
#define COCOAPODS_VERSION_PATCH_CocoaLumberjack_CLI 0

// CocoaLumberjack/Core
#define COCOAPODS_POD_AVAILABLE_CocoaLumberjack_Core
#define COCOAPODS_VERSION_MAJOR_CocoaLumberjack_Core 2
#define COCOAPODS_VERSION_MINOR_CocoaLumberjack_Core 0
#define COCOAPODS_VERSION_PATCH_CocoaLumberjack_Core 0

// CocoaLumberjack/Extensions
#define COCOAPODS_POD_AVAILABLE_CocoaLumberjack_Extensions
#define COCOAPODS_VERSION_MAJOR_CocoaLumberjack_Extensions 2
#define COCOAPODS_VERSION_MINOR_CocoaLumberjack_Extensions 0
#define COCOAPODS_VERSION_PATCH_CocoaLumberjack_Extensions 0

// JKLogDispatcher
#define COCOAPODS_POD_AVAILABLE_JKLogDispatcher
#define COCOAPODS_VERSION_MAJOR_JKLogDispatcher 1
#define COCOAPODS_VERSION_MINOR_JKLogDispatcher 0
#define COCOAPODS_VERSION_PATCH_JKLogDispatcher 1

// JKLogDispatcher/CocoaLumberjackModule
#define COCOAPODS_POD_AVAILABLE_JKLogDispatcher_CocoaLumberjackModule
#define COCOAPODS_VERSION_MAJOR_JKLogDispatcher_CocoaLumberjackModule 1
#define COCOAPODS_VERSION_MINOR_JKLogDispatcher_CocoaLumberjackModule 0
#define COCOAPODS_VERSION_PATCH_JKLogDispatcher_CocoaLumberjackModule 1

// JKLogDispatcher/Core
#define COCOAPODS_POD_AVAILABLE_JKLogDispatcher_Core
#define COCOAPODS_VERSION_MAJOR_JKLogDispatcher_Core 1
#define COCOAPODS_VERSION_MINOR_JKLogDispatcher_Core 0
#define COCOAPODS_VERSION_PATCH_JKLogDispatcher_Core 1

